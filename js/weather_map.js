(function () {
    ...
    document.getElementById('setMarkerButton').addEventListener('click', function(event) {
        event.preventDefault(); // Prevent the default form submission behavior

        const zipInputValue = document.getElementById('setMarker').value;

        if (event.key === 'Enter') {
            let zipInputValue = event.target.value; // Get the value from the input field

            // Do something with the input value
            console.log('The user input is:' + zipInputValue);
        }

        /** FORECASTS CURRENT WEATHER **/

        $.get(`https://api.openweathermap.org/data/2.5/weather?zip=${zipInputValue},us&appid=${WEATHERMAP_API_KEY}&units=metric`).done(data=> {
        const time = new Date();
        $("#currentWeather").html(`
    ...
    <div class="row">Wind: ${data.wind.speed} mph</div>
    ...
    $.get(`api.openweathermap.org/data/2.5/forecast/daily?zip=${zipInputValue},us&appid=${WEATHERMAP_API_KEY}&units=metric`).done(data=> {
    ...
    <div class="row">Temp: ${data.main.temp}°F</div>
    <div class="row no-gap">${data.main.temp_min}°F / ${data.main.temp_max}°F</div>
    <div class="row no-gap">Wind: ${data.wind.speed} mph</div>
    <div class="row no-gap">Humiditity: ${data.main.humidity}%</div>
    <div class="row no-gap">Air Pressure: ${data.main.pressure}</div>
    </div>
    `);
    });
    ...
    // <div class="row no-gap">Wind: ${day3.wind.speed} mph</div>
    ...
    // <div class="row no-gap">Wind: ${day4.wind.speed} mph</div>
    ...
    // <div class="row no-gap">Wind: ${day5.wind.speed} mph</div>
